...
class Model(nn.Module):
    def __init__(self, model_name, dataset, num_classes, is_fine_tuning=True, pretrained=True,
        ...
        if model_name.find('resnet') != -1: # ResNet-101 model.
            self.conv1 = prev_model.conv1
            self.bn1 = prev_model.bn1
            self.relu = prev_model.relu
            self.maxpool = prev_model.maxpool
            self.layer1 = prev_model.layer1
            self.layer2 = prev_model.layer2
            self.layer3 = prev_model.layer3
            self.layer4 = prev_model.layer4
            self.avgpool = prev_model.avgpool
            # self.classifier = prev_model.fc
        else:
    ...
    def load_model(self, dataset, model_name, network_name):
        ...
        if model_name.find('vgg16') != -1:  # vgg16 model.
    ...
    def params(self, lr, is_fine_tuning=True):
        if is_fine_tuning:
            if self.num_classifiers > 1:
                params = [{'params': self.features.parameters(), 'lr': 0.1 * lr},  # 0.01
                          {'params': self.fc6.parameters(), 'lr': 0.1 * lr},  # 0.01
                          {'params': self.fc7.parameters(), 'lr': 0.1 * lr}]  # 0.05
