...
class Model(nn.Module):
    def __init__(self, model_name, dataset, num_classes, is_fine_tuning=True, pretrained=True,
                 network_name='retroModel_no_cycle'):
...
class ExpModel(nn.Module):
    def __init__(self):
        ...
        prev_model = models.alexnet(pretrained=True)

        # Define the base model.
        self.pretrained_model = prev_model

        self.pretrained_model.classifier._modules['6'] = nn.Linear(prev_model.classifier._modules['6'].in_features, 67)

        checkpoint = torch.load('FT_scenes2')
        self.load_state_dict(checkpoint['state_dict'])  # Containing ['bias', 'weight'].
...
class ExpModel2(nn.Module):
    def __init__(self):
        ...
        self.fc6 = nn.Sequential(*list(prev_model.classifier.children())[:3])
        self.fc7 = nn.Sequential(*list(prev_model.classifier.children())[3:6])
        self.classifier0 = nn.Linear(prev_model.classifier._modules['6'].in_features, 67)
...
def train_model(model, optimizer, scheduler, start_epoch, num_epochs, dataloaders, dataset_sizes,
                model_name, dataset, num_class, mini_dataloaders, mini_dataset_sizes, ld=0.02):
    ...
    best_loss = 0.0
    best_acc, best_mean_acc = 0.0, 0.0
        ...
        for phase in ['train', 'test']:
            ...
            for i, data in enumerate(total_dataloader):
                ...
                if phase == 'train':
                    ...
                    loss = distillation_loss + retrospection_loss
                else:
