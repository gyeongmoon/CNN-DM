...
class ParseNet_Flowers(nn.Module):
    def __init__(self, is_fine_tuning, is_GB, is_WWA, num_classes=102, is_scale=True):
        ...
        if is_GB:  # WWA-CNN from "Growing a Brain" paper.
            ...
            if is_WWA:
                self.augClassifier = nn.Linear(512 * 7 * 7 + 4096, 2048)  # Both features and fc6.
            else:
...
class PNN_Three_Task_Model(nn.Module):
    def __init__(self, is_fine_tuning, num_classes=102, num_classes2=200):
        ...
        self.pretrained_model = PNN_VGG_Flowers(is_fine_tuning)
...
class PNN_VGG_Flowers(nn.Module):
    def __init__(self, is_fine_tuning, num_classes=102):
        super(PNN_VGG_Flowers, self).__init__()
        pretrained_model = models.vgg16(pretrained=True)
...
class EWC_Three_Task_Model(nn.Module):
    def __init__(self, is_fine_tuning, num_classes=102, num_classes2=200):
        ...
        self.pretrained_model = EWC_VGG_Flowers(is_fine_tuning)
...
class EWC_VGG_Flowers(nn.Module):
    def __init__(self, is_fine_tuning, num_classes=102):
        super(EWC_VGG_Flowers, self).__init__()
        pretrained_model = models.vgg16(pretrained=True)
...
def vgg_flowers(pretrained=False, is_fine_tuning=False, memory=True, is_GB=False, is_WWA=True,
                PNN=False, EWC=False, num_classes=102, num_classes2=200, task_index=1, **kwargs):
    """Pre-trained VGG-16 for Flowers dataset.
    ...
    if memory:
        ...
        if PNN:
            if task_index is 3:
                ...
                model = PNN_VGG_Flowers(is_fine_tuning, **kwargs)
        elif EWC:
            if task_index is 3:
                model = EWC_Three_Task_Model(is_fine_tuning, num_classes, num_classes2)
            else:
                model = EWC_VGG_Flowers(is_fine_tuning, **kwargs)
        else:
    ...
    if pretrained:
        if memory:
            if task_index is 3:
                if num_classes2 == 67:
                    checkpoint = torch.load('vgg_flowers_scenes_model')
                elif num_classes2 == 200:
                    checkpoint = torch.load('vgg_flowers_birds_model')
            else:
                checkpoint = torch.load('vgg_flowers_model')
        else:
            if PNN:
                if task_index is 3:
                    if num_classes2 == 67:
                        checkpoint = torch.load('pnn_vgg_flowers_scenes_model')
                    elif num_classes2 == 200:
                        checkpoint = torch.load('pnn_vgg_flowers_birds_model')
                else:
                    checkpoint = torch.load('pnn_vgg_flowers_model')
            if EWC:
                if task_index is 3:
                    if num_classes2 == 67:
                        checkpoint = torch.load('ewc_vgg_flowers_scenes_model')
                    elif num_classes2 == 200:
                        checkpoint = torch.load('ewc_vgg_flowers_birds_model')
                else:
                    checkpoint = torch.load('ewc_vgg_flowers_model')
            else:
                checkpoint = torch.load('vgg_flowers_model')
