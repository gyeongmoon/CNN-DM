...
class Model(nn.Module):
    def __init__(self, model_name, dataset, num_classes, GB=False, is_WWA=True, is_scale=True, guided_learning=True,
        ...
        for i, num_class in enumerate(num_classes):
            # if (i == 0) & ('imagenet' in dataset[i]):
            # if (i == 0) & ('scenes' in dataset[i]):
            if (i == 0) & ('flowers' in dataset[i]):
                # self.classifier0 = prev_model.classifier[6]
        ...
        if is_fine_tuning:
            if self.num_classifiers > 1:
                if self.GB:  # From here.
                    ...
                    for i in range(self.num_classifiers):
                        if i != self.num_classifiers - 1:
                            # if (i == 0) & (self.classifier0.out_features == 1000):
                            # if (i == 0) & (self.classifier0.out_features == 67):
                            if (i == 0) & (self.classifier0.out_features == 102):
                                params = params + [{'params': getattr(self, 'classifier' + str(i)).parameters(), 'lr': 0.001 * lr}]
                            else:
                                params = params + [{'params': getattr(self, 'classifier' + str(i)).parameters(), 'lr': 0.001 * lr},
    ...
    def forward(self, x):
        ...
        for i in range(self.num_classifiers):
            ...
            # if (i == 0) & (self.classifier0.out_features == 67):
            if (i == 0) & (self.classifier0.out_features == 102):
                output = self.classifier0(fc7)
                outputs = outputs + [output]
                augOutputs = augOutputs + [None]
                continue
