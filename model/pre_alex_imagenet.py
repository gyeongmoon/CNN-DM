...
class Three_Task_Model(nn.Module):
    def __init__(self, is_fine_tuning, is_GB, is_WWA, num_classes, num_classes2, is_scale=True):
        ...
        self.is_GB = is_GB
        self.is_WWA = is_WWA
        self.pretrained_model = ParseNet_ImageNet(is_fine_tuning, is_GB, is_WWA)
...
class PNN_Three_Task_Model(nn.Module):
    def __init__(self, is_fine_tuning, num_classes=102, num_classes2=200):
        ...
        self.pretrained_model = PNN_AlexNet_ImageNet(is_fine_tuning)
...
class PNN_AlexNet_ImageNet(nn.Module):
    def __init__(self, is_fine_tuning):
        super(PNN_AlexNet_ImageNet, self).__init__()
        pretrained_model = models.alexnet(pretrained=True)
        ...
        self.fc6 = nn.Sequential(*list(pretrained_model.classifier.children())[:3])
        self.fc7 = nn.Sequential(*list(pretrained_model.classifier.children())[3:6])
        self.classifier = pretrained_model.classifier._modules['6']
...
class EWC_Three_Task_Model(nn.Module):
    def __init__(self, is_fine_tuning, num_classes=102, num_classes2=200):
        ...
        self.pretrained_model = EWC_AlexNet_ImageNet(is_fine_tuning)
...
class EWC_AlexNet_ImageNet(nn.Module):
    def __init__(self, is_fine_tuning, num_classes=102):
        super(EWC_AlexNet_ImageNet, self).__init__()
        pretrained_model = models.alexnet(pretrained=True)
...
class Pre_AlexNet_ImageNet(nn.Module):
    def __init__(self, is_fine_tuning):
        super(Pre_AlexNet_ImageNet, self).__init__()
        pretrained_model = models.alexnet(pretrained=True)
...
def pre_alex_imagenet(pretrained=False, is_fine_tuning=False, memory=True, is_GB=False, is_WWA=True,
                     PNN=False, EWC=False, num_classes=102, num_classes2=200, task_index=1, **kwargs):
    """Pre-trained AlexNet for ImageNet dataset.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet.
        is_fine_tuning (bool): If True, returns a fine-tuning model, otherwise a feature-extraction model.
        memory (bool): If True, returns a ParseNet model pre-trained on ImageNet.
    """
    if memory:
        if task_index is 3:  # [num_classes, num_classes2] should be defined.
            ...
            model = ParseNet_ImageNet(is_fine_tuning, is_GB, is_WWA, **kwargs)
    else:
        if PNN:
            if task_index is 3:
                model = PNN_Three_Task_Model(is_fine_tuning, num_classes, num_classes2)
            else:
                model = PNN_AlexNet_ImageNet(is_fine_tuning, **kwargs)
                pretrained = False
        elif EWC:
            if task_index is 3:
                model = EWC_Three_Task_Model(is_fine_tuning, num_classes, num_classes2)
            else:
                model = EWC_AlexNet_ImageNet(is_fine_tuning, **kwargs)
        else:
            model = Pre_AlexNet_ImageNet(is_fine_tuning)
            pretrained = False
    ...
    if pretrained:
        if memory:
            if task_index is 3:
                if num_classes2 == 67:
                    checkpoint = torch.load('mem_imagenet_scenes_model')
                elif num_classes2 == 200:
                    checkpoint = torch.load('mem_imagenet_birds_model')
            else:
                checkpoint = torch.load('mem_imagenet_model')
        else:
            if PNN:
                if task_index is 3:
                    if num_classes2 == 67:
                        checkpoint = torch.load('pnn_imagenet_scenes_model')
                    elif num_classes2 == 200:
                        checkpoint = torch.load('pnn_imagenet_birds_model')
                else:
                    checkpoint = torch.load('pnn_alex_imagenet_model')
            if EWC:
                if task_index is 3:
                    if num_classes2 == 67:
                        checkpoint = torch.load('ewc_imagenet_scenes_model')
                    elif num_classes2 == 200:
                        checkpoint = torch.load('ewc_imagenet_birds_model')
                else:
                    checkpoint = torch.load('ewc_alex_imagenet_model')
            else:
                checkpoint = torch.load('pre_alex_imagenet_model')
